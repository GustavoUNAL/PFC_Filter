...
fft_ln, = ax2.plot([], [], 'b-', animated=True)
...
def init():
    ax1.set_xlim(0, window_duration)  # Ajusta el límite del eje x para la ventana de tiempo
    ax1.set_ylim(df['Sine Wave3'].min(), df['Sine Wave3'].max())  # Ajusta el límite del eje y basado en los datos
    ax2.set_xlim(0, 500)  # Ajusta el límite del eje x para la FFT (frecuencia)
    ax2.set_ylim(0, 1)  # Ajusta el límite del eje y para la FFT (magnitud)
    return ln, fft_ln
...
def update(frame):
    ...
    ax1.set_xlim(xdata[0], xdata[-1])  # Ajustar el límite del eje x dinámicamente
    ln.set_data(xdata, ydata)
    
    # Calcular la FFT de los datos en la ventana de tiempo actual
    if len(ydata) > 1:
        yf = np.fft.fft(ydata)
        xf = np.fft.fftfreq(len(ydata), (xdata[1] - xdata[0]))[:len(ydata)//2]
        fft_ln.set_data(xf, 2.0/len(ydata) * np.abs(yf[:len(ydata)//2]))
